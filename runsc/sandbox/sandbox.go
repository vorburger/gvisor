...
type Args struct {
	...
	// RootFileStoreFD is the regular file FD that will back the tmpfs upper
	// mount in the root overlay mount.
	RootFileStoreFD *os.File
...
func (s *Sandbox) createSandboxProcess(conf *config.Config, args *Args, startSyncFile *os.File) error {
	...
	donations.DonateAndClose("root-filestore-fd", args.RootFileStoreFD)
