...
func (rf *regularFile) Translate(ctx context.Context, required, optional memmap.MappableRange, at hostarch.AccessType) ([]memmap.Translation, error) {
	...
	if translatedEnd < required.End && cerr != nil {
		rf.inode.fs.unaccountPages(pagesReqd)
...
func (rw *regularFileReadWriter) WriteFromBlocks(srcs safemem.BlockSeq) (uint64, error) {
	...
	for rw.off < end {
		...
		case gap.Ok():
			...
			if !rw.file.inode.fs.accountPages(gapMR.Length() / hostarch.PageSize) {
				return 0, linuxerr.ENOSPC
			}
			fr, err := rw.file.memFile.Allocate(gapMR.Length(), pgalloc.AllocOpts{Kind: rw.file.memoryUsageKind})
			if err != nil {
				retErr = err
				rw.file.inode.fs.unaccountPages(gapMR.Length() / hostarch.PageSize)
				goto exitLoop
			}
